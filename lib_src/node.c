...
static int L_getNodeName(lua_State *L)
{
	lua_pushstring(L, ixmlNode_getNodeName(checknode(L, 1)));
	return 1;
}

static int L_getNodeValue(lua_State *L)
{
	lua_pushstring(L, ixmlNode_getNodeValue(checknode(L, 1)));
	return 1;
}

static int L_setNodeValue(lua_State *L)
{
...
static int L_getPrefix(lua_State *L)
{
...
static int L_getLocalName(lua_State *L)
{
...
static int L_insertBefore(lua_State *L)
{
...
static int L_appendChild(lua_State *L)
{
	IXML_Node* addtonode = checknode(L, 1);
	IXML_Node* newchild = checknode(L, 2);
	int result;
...
static int L_hasChildNodes(lua_State *L)
{
...
static int L_cloneNode(lua_State *L)
{
	pushLuaNode(L, ixmlNode_cloneNode(checknode(L,1), lua_toboolean(L,2)));
	return 1;
}

static int L_hasAttributes(lua_State *L)
{
